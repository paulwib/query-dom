...
Element.prototype.isHidden = function () {
  if (this.hasAttribute('hidden') && this.getAttribute('hidden') !== 'false') return true;
  if (this.hasAttribute('aria-hidden') && this.getAttribute('aria-hidden') !== 'false') return true;
...
getter('innerText', function () {
  return this.childNodes
    .filter(child => child.nodeName === '#text' ? true : !child.isHidden())
    .map(child => child.nodeName === '#text' ? child.textContent : child.innerText)
    .join('');
});
